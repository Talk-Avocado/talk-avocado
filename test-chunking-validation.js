...
async function testChunkingValidation() {
  ...
  for (const testFile of testFiles) {
    ...
    logger.info(
      `Duration: ${testFile.duration} seconds (${(testFile.duration / 60).toFixed(2)} minutes)`
    );
      ...
      if (finalManifest.transcript && finalManifest.transcript.key) {
        ...
        if (existsSync(transcriptPath)) {
          ...
          if (transcriptData.segments && transcriptData.segments.length > 0) {
            ...
            logger.info(
              `  - Total transcript duration: ${lastSegment.end.toFixed(2)}s (${(lastSegment.end / 60).toFixed(2)} min)`
            );
            ...
            const timestampAccuracy =
              gaps === 0 && overlaps === 0 ? "✅ PASS" : "⚠️  WARN";
            // eslint-disable-next-line no-unused-vars
            const durationAccuracyResult =
              durationDiff < 5 ? "✅ PASS" : "⚠️  WARN"; // 5 second tolerance
  ...
  for (const result of results) {
    ...
    logger.info(
      `  Duration: ${result.duration}s (${(result.duration / 60).toFixed(2)} min)`
    );
    ...
    if (result.success) {
      logger.info(
        `  Transcription time: ${result.transcriptionTime.toFixed(2)}s`
      );
      logger.info(`  Segments: ${result.segments}`);
      logger.info(
        `  Transcript duration: ${result.transcriptDuration.toFixed(2)}s`
      );
