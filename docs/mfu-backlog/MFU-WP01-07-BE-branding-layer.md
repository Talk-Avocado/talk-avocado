...
- Two-step rendering: `video-cuts` then optional `video-transitions`; branding consumes `with_transitions.mp4` or falls back to `base_cuts.mp4`.
- Canonical artefacts: final output at `renders/final.mp4`; subtitles at `subtitles/final.srt` (optional `.vtt`).
- Orchestration via AWS Step Functions (Standard); handler event matches ASL Task input.
- Manifest governance: semver schema, schema owner, and `extra.branding.*`; validate on every write.
- Media policy: H.264 High, CRF 20; aspect ratio preserved (letterbox/pillarbox), loudness −16 LUFS ±1; default crossfade 500 ms.
...
- Behavior:
  ...
  - Write `renders/final.mp4` with branding applied
...
- [ ] Output `renders/final.mp4` is produced with all branding applied
...
4) Implement handler
    ...
    exports.handler = async (event, context) => {
      ...
      const outputKey = keyFor(env, tenantId, jobId, 'renders', 'final.mp4');
...
- Run harness on a short input with branding assets:
  - Expect `renders/final.mp4` with applied branding
