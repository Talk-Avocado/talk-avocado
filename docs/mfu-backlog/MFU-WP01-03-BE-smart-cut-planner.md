...
- Writes `plan/cut_plan.json` and sets manifest pointer; clear inputs for cuts.
- Deterministic output with `DETERMINISTIC=true`; golden comparisons rely on byte equality.
- Standard metrics: `PlanningSuccess`, `TotalCuts`, `PlanningError_{Type}`; error taxonomy standardized.
