...
async function runPerformanceTest() {
  ...
  const existingAudioKey = keyFor(env, tenantId, existingJobId, 'audio', `${existingJobId}.mp3`);
  ...
  if (!existsSync(existingAudioPath)) {
    ...
    logger.error('  node tools/harness/run-local-pipeline.js --input podcast-automation/test-assets/raw/sample-short.mp4 --env dev --tenant t-perf');
    process.exit(1);
  }
  ...
  const audioKeyCtranslate = keyFor(env, tenantId, testJobIdCtranslate, 'audio', `${testJobIdCtranslate}.mp3`);
  ...
  const audioKeyStandard = keyFor(env, tenantId, testJobIdStandard, 'audio', `${testJobIdStandard}.mp3`);
  ...
  try {
    ...
    logger.info('Note: Handler will auto-detect python -m whisper if whisper command fails');
  ...
  if (standardTime) {
    ...
    const percentFaster = ((standardTime - ctranslateTime) / standardTime) * 100;
    ...
    if (speedup >= 2.0) {
      ...
      logger.info(`   Speedup: ${speedup.toFixed(2)}x (${percentFaster.toFixed(1)}% faster)`);
      return true;
    } else {
      logger.warn(`⚠️ Performance target NOT met: Expected 2x+ speedup, got ${speedup.toFixed(2)}x`);
      logger.warn(`   Improvement: ${percentFaster.toFixed(1)}% faster (need 100%+ for 2x)`);
      return false;
    }
  } else {
    logger.info('Standard Whisper: Not available for comparison');
    logger.info('');
    logger.info('whisper-ctranslate2 performance:', `${(ctranslateTime / 1000).toFixed(2)}s`);
    logger.info('');
...
async function runPhase1Tests() {
  ...
  const tests = [
    { name: 'Timestamp Alignment', file: 'test-timestamp-alignment.js', arg: 'f0284511-c163-476d-ada1-e7ed2b105e45' },
    { name: 'Idempotency', file: 'test-idempotency-repeat-runs.js' },
    { name: 'Error - Missing Audio', file: 'test-error-missing-audio.js' },
    { name: 'Error - Missing Audio Key', file: 'test-error-missing-audio-key.js' },
    { name: 'Error - Whisper Not Installed', file: 'test-error-whisper-not-installed.js' },
...
async function main() {
  try {
    ...
    if (perfResult === true) {
      ...
      logger.info('ℹ️ Performance Test: SKIPPED (standard whisper not available)');
    }
