...
function checkRequireStatements() {
  ...
  const jsFiles = findFiles('.', ['.js', '.ts']).filter(file => 
    !file.includes('.cjs') && 
  ...
  if (violations.length > 0) {
    ...
    log('   Example: const fs = require(\'fs\') → import { readFile } from \'fs\'', 'yellow');
    return false;
  }
...
function checkPackageJsonFiles() {
  ...
  const packageFiles = findFiles('.', ['.json']).filter(file => 
    file.endsWith('package.json') && !file.includes('node_modules') && !file.includes('dist')
  );
