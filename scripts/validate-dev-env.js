...
for (const pkg of requiredPackages) {
  const check = checkCommand(`npm list -g ${pkg}`, `${pkg} global installation`);
  if (check.success) {
...
if (checkFileExists('backend/package.json', 'Backend package.json')) {
  if (checkCommand('cd backend && npm ci', 'Backend dependencies installation').success) {
...
if (checkFileExists('.eslintrc.cjs', 'ESLint configuration')) {
  const eslintCheck = checkCommand('npx eslint --print-config .eslintrc.cjs', 'ESLint configuration validation');
...
const esModuleCheck = checkCommand('npm run check-es-modules', 'ES module compliance check');
