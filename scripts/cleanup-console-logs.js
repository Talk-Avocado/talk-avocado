...
function cleanupConsoleLogs(filePath) {
  try {
    ...
    if (
      ...
      content.includes('from "../../scripts/logger.js"')
    ) {
    ...
    if (consoleLogMatches) {
      ...
      modified = true;
      changes.push(
        `Replaced ${consoleLogMatches.length} console.log with logger.info`
      );
    }
    ...
    if (consoleErrorMatches) {
      ...
      modified = true;
      changes.push(
        `Replaced ${consoleErrorMatches.length} console.error with logger.error`
      );
    }
    ...
    if (consoleWarnMatches) {
      ...
      modified = true;
      changes.push(
        `Replaced ${consoleWarnMatches.length} console.warn with logger.warn`
      );
    }
...
function main() {
  ...
  const jsFiles = findFiles(".", [".js"]).filter(
    file =>
  ...
  if (totalModified > 0) {
    ...
    log(
      "   2. Test the modified files to ensure they work correctly",
      "yellow"
    );
    log(
      "   3. Consider removing remaining console.log in test files if not needed",
      "yellow"
    );
  } else {
