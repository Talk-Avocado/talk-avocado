...
async function testPerformanceQuick() {
  ...
  const longAudioPath =
    "storage/dev/t-perf/012a43c4-bfbe-411b-aeb2-18feeda15255/audio/012a43c4-bfbe-411b-aeb2-18feeda15255.mp3";
  ...
  if (existsSync(shortSegmentPath)) {
    ...
    logger.info(
      "Short segment not found, will use first 2 minutes of long file"
    );
    logger.info(
      "Note: For quick testing, consider extracting a short segment first"
    );
  ...
  logger.info("");
  logger.info(
    "Note: For full performance comparison (2x+ speedup verification),"
  );
