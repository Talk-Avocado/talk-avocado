    ...
    if (Array.isArray(validatedCuts) && validatedCuts.length > 0) {
      ...
      validatedCuts = validatedCuts.filter(cut => {
        if (/(dead-air|pause)/i.test(cut.reason)) {
          ...
          if (process.env.LOCAL_MODE === "true") {
            const audioPath = require("path").resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", `mp4/${base}.mp4`);
...
if (process.env.LOCAL_MODE === "true") {
  const { resolve } = require("path");
  const audioPath = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", `mp4/${base}.mp4`);
...
cutRanges = finalFailSafe(cutRanges, vidDuration);
      ...
      if (process.env.LOCAL_MODE === "true") {
        const { writeFileSync, mkdirSync } = require("fs");
        const { resolve, dirname } = require("path");
        const localPath = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", cutplanKey);
        mkdirSync(dirname(localPath), { recursive: true });
