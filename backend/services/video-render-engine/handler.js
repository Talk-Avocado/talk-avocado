  ...
  .map(({ start, end, reason }) => ({ start, end, reason }));
    ...
    try {
      ...
      if (process.env.LOCAL_MODE === "true") {
        const { readFileSync, existsSync } = require("fs");
        const { resolve } = require("path");
        const localPath = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", polishedKey);
        if (existsSync(localPath)) {
    ...
    if (process.env.LOCAL_MODE === "true") {
      ...
      const { resolve } = require("path");
      const localSource = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", inputKey);
    ...
    try {
      const ffprobeCmd = `ffprobe -v error -show_entries format=duration -of default=nokey=1:noprint_wrappers=1 "${inputPath}"`;
...
if (!concatSucceeded) {
  ...
  execSync(ffmpegConcatReencodeCmd, { stdio: "inherit" });
    ...
    if (process.env.LOCAL_MODE === "true") {
      ...
      const localDest = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", outputKey);
...
async function copyOriginalVideo(bucket, cutplanKey) {
  ...
  if (process.env.LOCAL_MODE === "true") {
    const { copyFileSync, mkdirSync } = require("fs");
    const { resolve, dirname } = require("path");
    const localSource = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", originalKey);
    const localDest = resolve(__dirname, "..", "..", "..", "podcast-automation", "test-assets", outputKey);
    mkdirSync(dirname(localDest), { recursive: true });
