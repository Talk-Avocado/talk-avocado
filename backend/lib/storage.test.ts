...
describe('Storage utilities', () => {
  ...
  test('key() joins parts with forward slashes', () => {
    ...
    assert.strictEqual(key('env', 'tenant', 'job', 'file.txt'), 'env/tenant/job/file.txt');
  });

  test('keyFor() creates tenant-scoped keys', () => {
    assert.strictEqual(keyFor('dev', 'tenant1', 'job1', 'manifest.json'), 'dev/tenant1/job1/manifest.json');
    assert.strictEqual(keyFor('prod', 'tenant2', 'job2', 'audio', 'file.mp3'), 'prod/tenant2/job2/audio/file.mp3');
  });
