...
import { keyFor, writeFileAtKey, readFileAtKey, pathFor } from "./storage.js";
...
describe("Integration: End-to-end local flow", () => {
  ...
  test("Complete job workflow: create job → write artifacts → update manifest → read back", async () => {
    ...
    const loadedTranscript = JSON.parse(
      readFileAtKey(transcriptKey).toString()
    );
    ...
    for (const expectedPath of expectedPaths) {
      assert(
        ...
        `Expected file to exist: ${expectedPath}`
      );
    }
