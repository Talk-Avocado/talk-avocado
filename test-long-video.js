...
const LONG_VIDEO = "podcast-automation/test-assets/raw/Weekly Q&A Session - 2025-07-11 - Includes Rachel discussing certified ip.mp4";
...
async function testLongVideo() {
  ...
  const inputKey = keyFor(TEST_ENV, TEST_TENANT, jobId, "input", path.basename(LONG_VIDEO));
  ...
  logger.info(`  Keep segments: ${cutPlan.cuts.filter(c => c.type === "keep").length}`);
  logger.info(`  Cut segments: ${cutPlan.cuts.filter(c => c.type === "cut").length}`);
  logger.info("");

  // Prepare event
  const sourceVideoKey = keyFor(TEST_ENV, TEST_TENANT, jobId, "input", path.basename(LONG_VIDEO));
  const event = {
    env: TEST_ENV,
    tenantId: TEST_TENANT,
    jobId: jobId,
