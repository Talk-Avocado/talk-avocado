...
export class TalkAvocadoLambdaStack extends cdk.Stack {
  ...
  private createCloudWatchAlarms(config: any) {
    // Error rate alarm
    Object.entries(this.functions).forEach(([serviceName, func]) => {
      const errorRateAlarm = new cdk.aws_cloudwatch.Alarm(this, `${serviceName}-ErrorRate-Alarm`, {
      ...
      // Duration alarm
      const durationAlarm = new cdk.aws_cloudwatch.Alarm(this, `${serviceName}-Duration-Alarm`, {
      ...
      const dlqAlarm = new cdk.aws_cloudwatch.Alarm(this, `${serviceName}-DLQ-Alarm`, {
