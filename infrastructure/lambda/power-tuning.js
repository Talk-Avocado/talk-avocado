...
const MEMORY_TIERS = [
  128, 256, 512, 1024, 1536, 1769, 2048, 3008, 4096, 5120, 6144, 7168, 8192, 9216, 10240
];
...
class LambdaPowerTuner {
   ...
   */
  analyzeResults() {
    const successfulResults = this.results.filter(r => r.success && r.duration && r.cost);
  ...
  generateReport() {
    ...
    console.log('\n🏆 RECOMMENDATIONS:');
    console.log(`💰 Most Cost-Effective: ${this.analysis.costEffective.memory}MB`);
    ...
    const currentResult = this.results.find(r => r.memory === currentConfig.currentMemory);
    ...
    if (currentResult && currentResult.success) {
      ...
      if (improvement < 1) {
        console.log(`   💡 Potential cost savings: ${((1 - improvement) * 100).toFixed(1)}%`);
      } else {
        console.log(`   ⚠️  Current config is ${((improvement - 1) * 100).toFixed(1)}% more expensive`);
      }
    }
