...
class InitDurationTester {
  ...
  async testInitDuration(iteration = 1) {
    ...
    try {
      ...
      console.log(
        `   ${status} Init time: ${initTime}ms (Total: ${totalTime}ms)`
      );
  ...
  async simulateInitProcess() {
    ...
    const phases = [
      ...
      { name: "Runtime initialization", duration: 500 + Math.random() * 300 },
      {
        ...
        duration: this.simulateDependencyLoading(service.dependencies),
      },
  ...
  simulateFFmpegInit() {
    ...
    if (
      this.serviceName.includes("video") ||
      this.serviceName.includes("audio")
    ) {
  ...
  analyzeResults() {
    ...
    const variance =
      initTimes.reduce(
        (sum, time) => sum + Math.pow(time - avgInitTime, 2),
        0
      ) / initTimes.length;
  ...
  generateReport() {
    ...
    console.log(
      `Performance Category: ${this.analysis.performanceCategory.toUpperCase()}`
    );
    ...
    const coefficientOfVariation =
      (this.analysis.stdDev / this.analysis.avgInitTime) * 100;
...
async function main() {
  ...
  if (!serviceName || !SERVICES[serviceName]) {
    console.log(
      "Usage: node init-duration-test.js <service-name> [iterations]"
    );
