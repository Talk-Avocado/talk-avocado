  ...
  await withDelay(async () => {
    try {
      const params = { logGroupName },
        command = new Logs.CreateLogGroupCommand(params);
...
async function deleteLogGroup(logGroupName, client, withDelay) {
  await withDelay(async () => {
    try {
      const params = { logGroupName },
        command = new Logs.DeleteLogGroupCommand(params);
      await client.send(command);
    } catch (error) {
  ...
  await withDelay(async () => {
    if (retentionInDays) {
      const params = { logGroupName, retentionInDays },
        putCommand = new Logs.PutRetentionPolicyCommand(params);
      await client.send(putCommand);
    } else {
      const params = { logGroupName },
        deleteCommand = new Logs.DeleteRetentionPolicyCommand(params);
...
async function handler(event, context) {
  try {
    ...
    const logGroupName = event.ResourceProperties.LogGroupName,
      logGroupRegion = event.ResourceProperties.LogGroupRegion,
      maxRetries =
        parseIntOptional(event.ResourceProperties.SdkRetry?.maxRetries) ?? 5,
      withDelay = makeWithDelay(maxRetries),
      sdkConfig = {
        logger: console,
        region: logGroupRegion,
        maxAttempts: Math.max(5, maxRetries),
      },
      client = new Logs.CloudWatchLogsClient(sdkConfig);
    if (
  ...
  function respond(responseStatus, reason, physicalResourceId) {
    const responseBody = JSON.stringify({
      ...
      Data: { LogGroupName: event.ResourceProperties.LogGroupName },
    });
...
function parseIntOptional(value, base = 10) {
  if (value !== void 0) return parseInt(value, base);
}
function makeWithDelay(maxRetries, delayBase = 100, delayCap = 10 * 1e3) {
  return async block => {
    let attempts = 0;
    do
      try {
          ...
          if (attempts < maxRetries) {
            (attempts++,
              await new Promise(resolve =>
                setTimeout(
                  resolve,
                  calculateDelay(attempts, delayBase, delayCap)
                )
              ));
            continue;
