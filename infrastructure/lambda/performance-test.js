...
class PerformanceTester {
  ...
  async runPerformanceTests() {
    console.log(
      `🚀 Starting comprehensive performance testing for ${this.serviceName}...`
    );
  ...
  generateCombinedAnalysis() {
    ...
    if (!powerAnalysis || !initAnalysis) {
      console.log(
        "❌ Incomplete test results - cannot generate combined analysis"
      );
      return;
    }
    ...
    console.log(
      `Performance Category: ${initAnalysis.performanceCategory.toUpperCase()}`
    );
  ...
  calculateMemoryScore(powerAnalysis) {
    ...
    const memoryRatio =
      Math.min(recommendedMemory, currentMemory) /
      Math.max(recommendedMemory, currentMemory);
  ...
  calculateInitScore(initAnalysis) {
    ...
    if (avgInitTime <= expectedTime) {
      ...
      return Math.max(
        0,
        60 - ((avgInitTime - criticalTime) / criticalTime) * 40
      );
    }
  }
  ...
  calculateConsistencyScore(initAnalysis) {
    const coefficientOfVariation =
      (initAnalysis.stdDev / initAnalysis.avgInitTime) * 100;
  ...
  calculateCostScore(powerAnalysis) {
    ...
    const currentMemory = this.getCurrentMemory();
    const currentResult = powerAnalysis.results.find(
      r => r.memory === currentMemory
    );

    if (!currentResult || !currentResult.success) return 50;

    const costImprovement =
      (currentResult.cost - powerAnalysis.balanced.cost) / currentResult.cost;
  ...
  calculateMemoryImprovement(powerAnalysis) {
    ...
    return (
      ((currentResult.cost - powerAnalysis.balanced.cost) /
        currentResult.cost) *
      100
    );
  }
  ...
  generateCombinedRecommendations(
    ...
    if (performanceScore >= 90) {
      ...
      console.log(
        "✅ GOOD: Configuration is well-optimized with room for improvement"
      );
    ...
    console.log(
      "2. Monitor init duration and implement optimizations if needed"
    );
