// tools/harness/compare-goldens.cjs
...
function compareMetrics(actualManifest, goldenMetrics, strict) {
  ...
  if (goldenMetrics.transcript) {
    let actual = actualManifest.transcript?.wordCount || 0;
    
    // If wordCount is not in manifest, try to calculate from transcript file
    if (actual === 0 && actualManifest.transcript?.jsonKey) {
      try {
        const transcriptPath = join(actualPath, 'transcripts', 'transcript.json');
        if (existsSync(transcriptPath)) {
          const transcript = JSON.parse(readFileSync(transcriptPath, 'utf-8'));
          const text = transcript.segments?.map(s => s.text).join(' ') || '';
          actual = text.split(/\s+/).filter(word => word.length > 0).length;
        }
      } catch (error) {
        // Ignore errors, use 0
      }
...
function compareManifestSubset(actual, golden) {
  ...
  for (const [key, expectedValue] of Object.entries(golden)) {
    ...
    // For nested objects, only compare the specified fields
    if (typeof expectedValue === 'object' && expectedValue !== null && !Array.isArray(expectedValue)) {
      if (typeof actualValue === 'object' && actualValue !== null) {
        for (const [subKey, subExpectedValue] of Object.entries(expectedValue)) {
          const subActualValue = actualValue[subKey];
          if (JSON.stringify(subActualValue) !== JSON.stringify(subExpectedValue)) {
            failures.push(`manifest.${key}.${subKey}: expected ${JSON.stringify(subExpectedValue)}, got ${JSON.stringify(subActualValue)}`);
          }
        }
      } else {
      ...
      // For arrays, compare the structure but allow additional elements
      if (Array.isArray(actualValue)) {
        for (let i = 0; i < expectedValue.length; i++) {
          ...
          if (typeof expectedItem === 'object' && expectedItem !== null) {
            for (const [subKey, subExpectedValue] of Object.entries(expectedItem)) {
              const subActualValue = actualItem?.[subKey];
              if (JSON.stringify(subActualValue) !== JSON.stringify(subExpectedValue)) {
                failures.push(`manifest.${key}[${i}].${subKey}: expected ${JSON.stringify(subExpectedValue)}, got ${JSON.stringify(subActualValue)}`);
              }
            }
          } else if (JSON.stringify(actualItem) !== JSON.stringify(expectedItem)) {
      ...
      // For primitive values, exact match
